...
int main() {
	...
	for (std::string line = ""; std::getline(ifs, line);) {
		...
		// Now we can sort the unsorted pages
		if (inOrder) continue;
		std::sort(pages.begin(), pages.end(), [&](int a, int b) {
			if (!invalidOrderingMap.contains(a)) return true;
			if (invalidOrderingMap.at(a).contains(b)) return false;
			return true;
			});
		sumOfMiddleNumbers += pages.at((pages.size() - 1) / 2);
	}
